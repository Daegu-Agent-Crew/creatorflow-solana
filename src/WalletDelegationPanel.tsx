...
import { confirmWalletDelegation, confirmWalletDelegationRevocation, getAgentSession, getWalletDelegation, listCampaigns, type Campaign, type WalletDelegation } from './api'
...
export function WalletDelegationPanel({ refreshKey }: { refreshKey: number }) {
  ...
  useEffect(() => {
    ...
    listCampaigns().then(async (result) => {
      const available = result.campaigns
      setCampaigns(available)
      const selected = available[0]?.id || ''
      setCampaignId(selected)
      if (selected) setDelegation((await getWalletDelegation(current, selected)).delegation)
    }).catch(() => setDelegation(null))
  }, [refreshKey, sessionAgentId, sessionRole])

  useEffect(() => {
    const current = getAgentSession()
    if (!current || current.role !== 'brand' || !campaignId) return
    getWalletDelegation(current, campaignId).then((result) => setDelegation(result.delegation)).catch(() => setDelegation(null))
  }, [campaignId])
  ...
  async function approve() {
    ...
    if (!campaignId) { setMessage('먼저 캠페인을 선택해 주세요.'); return }
    ...
    try {
      ...
      const confirmed = await confirmWalletDelegation(current, { campaignId, ownerWallet: wallet, transactionSignature: result.transactionSignature, allowanceBaseUnits })
  ...
  async function revoke() {
    ...
    try {
      ...
      const confirmed = await confirmWalletDelegationRevocation(delegation.delegationId, signature)
      setDelegation(confirmed.delegation)
    ...
    <article className="wallet-delegation-panel">
      ...
      <div className="delegation-content">
        <div>
          ...
          {session?.role === 'brand' && campaigns.length ? <label className="delegation-campaign"><span>적용 캠페인</span><select value={campaignId} onChange={(event) => setCampaignId(event.target.value)}>{campaigns.map((campaign) => <option value={campaign.id} key={campaign.id}>{campaign.title}</option>)}</select></label> : null}
        </div>
